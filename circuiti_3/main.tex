...
%TODO si dovrebbe aggiungere una footnote coi dati di questa affermazione
...
\begin{figure}[htbp] % Opzioni di posizionamento: h=here, t=top, b=bottom, p=page
    \centering % Centra l'intero contenuto della figura nella pagina

    % --- Prima Riga di Grafici ---
    \begin{subfigure}[b]{0.495\textwidth} % [b] allinea le didascalie in basso
        \centering
        \includegraphics[width=\linewidth]{grafici/rc_hc.pdf} % Sostituisci con il percorso reale
        \caption{Fit modulo $|H_C(\omega)|$}
        \label{fig:rc_hc}
    \end{subfigure}
    \hfill % Aggiunge spazio orizzontale flessibile tra le subfigure
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \includegraphics[width=\linewidth]{grafici/rc_hr.pdf} % Sostituisci con il percorso reale
        \caption{Fit modulo $|H_R(\omega)|$}
        \label{fig:rc_hr}
    \end{subfigure}

    \vspace{\baselineskip} % Aggiunge un po' di spazio verticale tra le righe (opzionale)

    % --- Seconda Riga di Grafici ---
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \includegraphics[width=\linewidth]{grafici/rc_fase_hc.pdf} % Sostituisci con il percorso reale
    ...
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        \includegraphics[width=\linewidth]{grafici/rc_fase_hr.pdf} % Sostituisci con il percorso reale
...
\begin{figure}[htbp] % Opzioni di posizionamento: h=here, t=top, b=bottom, p=page
    ...
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        % RL Low-Pass Phase (Resistor Voltage Phase)
    ...
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        % RL High-Pass Phase (Inductor Voltage Phase)
...
Basandoci sui dati ottenuti mediante il test del chi quadro possiamo affermare che le leggi attese non offrono un buon accordo con i dati ottenuti sperimentalmente nè per quanto riguarda i moduli delle funzioni di trasferimento $H_R(\omega)$ e $H_L(\omega)$ nè per quanto riguarda le fasi di tali funzioni. Ciò è imputabile al fatto che l'oscilloscopio contenesse una capacità interna molto piccola, che va ad aggiungersi in parallelo al circuito RL. Considerando l'impedenza totale del circuito vale quindi la relazione: $\frac{1}{Z_{eq}}=\frac{1}{Z_R+Z_L}+\frac{1}{Z_C}$ con $C<<R$ e $C<<L$. Per $\omega$ basse si ha $\frac{1}{Z_C}=j\omega C \approx 0 $ e dunque $Z_{eq} \approx Z_R+Z_L$, cioè il circuito si comporta come un circuito RL puro e le formule ricavate sono valide. Quando invece si hanno $\omega$ elevate il termine $\frac{1}{Z_C}=j\omega C$ non è più trascurabile e il circuito non si comporta più come un circuito composto esclusivamente da una resistenza ed un'induttanza in serie: bisogna tenere conto anche della presenza di una capacità $C$ in parallelo al suo interno. Per tale motivo abbiamo deciso di ripetere le interpolazioni dei dati scartando gli ultimi punti, ovvero quelli corrispondenti a frequenze alte, superiori ai 
 ...
 %TODO motivare perchè le fasi di rl fanno cagare
...
\begin{align}
	 ...
	 |H_R(\omega)| = \frac{R}{\sqrt{R^2 + \left(\omega L-\frac{1}{\omega C}\right)^2}} \qquad & \text{,} \qquad \arg H_R(\omega) = -\arctan\left(\frac{\omega L -\frac{1}{\omega C}}{R}\right) \label{eq:Vr_RLC} % Corrected WC to omega C
\end{align}
...
\begin{figure}[htbp] % Opzioni di posizionamento: h=here, t=top, b=bottom, p=page
    ...
    % --- Prima Riga di Grafici (RLC) ---
    \begin{subfigure}[b]{0.495\textwidth} % [b] allinea le didascalie in basso
        \centering
        % RLC Resistor Voltage Magnitude
    ...
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        % RLC Inductor Voltage Magnitude
    ...
    % --- Seconda Riga di Grafici (RLC) ---
    \begin{subfigure}[b]{0.495\textwidth}
        \centering
        % RLC Capacitor Voltage Magnitude
