...
statistica = perform_fit(scarica_tempi, scarica_tensione, func=V_l, p0=[0.500, 0.0010, -1], parameter_names=params)
#plot_fit(statistica, plot_residuals=True, n_fit_points=10000, xlabel='tempi', show_stats=True, show_plot=True, show_params=True, title='Circuito RL - Scarica Induttore')
latex_table_fit(statistica, orientation='v')
tabella_1 = latex_table_data(scarica_tempi, scarica_tensione, orientation='v', caption='Circuito RL - Scarica Induttore')
L_1 = statistica.parameters['tau']*R_eq
...
carica_tempi = Measurement(carica_tempi_induttore, 0.01, magnitude=-3, unit='s', name='carica: tempo') - 0.00305
carica_tensione = Measurement(carica_tensione_induttore, 2, magnitude=-3, unit='V', name='carica: tensione') + 0.248
statistica = perform_fit(carica_tempi, carica_tensione, V_l, [-0.500, 0.0010, 1.0],)
#plot_fit(statistica, plot_residuals=True, n_fit_points=10000, show_params=True, show_plot=True, show_stats=True, title='Circuito RL - Carica Induttore')
tabella_2 = latex_table_data(carica_tempi, carica_tensione, orientation='v', caption='Circuito RL - Carica Induttore')
L_2 = statistica.parameters['tau'] * R_eq
print(L_1, L_2)
print(test_comp(L_1, L_2)['interpretation'])
#############################################################################################################################################################################
